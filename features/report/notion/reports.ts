  ...
  try {
      ...
      properties: {
        ...
        'Location Code': {
          rich_text: [
            {
              text: {
                content: locationCode.trim(), // Trim to avoid whitespace issues
              },
            },
          ],
        },
    ...
    const baseUrl =
      ...
      (process.env.VERCEL_URL
        ...
        : 'http://localhost:3000');
    const pdfUrl = `${baseUrl}/api/report/${response.id}/pdf`;

    // Build PDF filename with location code and monthly indicator
    const locationPrefixForFile = locationCode ? `${locationCode.trim()}_` : '';
...
export async function getReportsFromNotion(
  ...
  // Filter only by locationCode if provided, otherwise get all reports
  ...
  try {
    ...
    // Only add filter if locationCode is provided
    if (filter) {
    ...
    return allResults.map((page: any) => {
      ...
      const locationCodeProp =
        props['Location Code']?.rich_text?.[0]?.text?.content || '';
      const userNameProp =
        props['User Name']?.rich_text?.[0]?.text?.content || '';
      ...
      const report = {
        ...
        userName: userNameProp || undefined,
